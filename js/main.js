window.addEventListener('DOMContentLoaded', function(){
  //==== html document 요소가 모두 로드된 후 실행될 코드 : gsap 가 자체적으로 생성하는 요소를 제어하기 위한 코드
  //====
  ...
  $('.cursor_effect')
    ...
    ScrollTrigger.create({
      ...
      scrub: 1.8, //마우스 이동후 .8초 지연
    });

    tit.add('s1') // s1 이라는 이름을 지어서 첫 번째 효과로 지정
    .to('.main_title_01', {opacity: 0, y: -100, duration: 4}, 's1')

    tit.add('s2') // s2 이라는 이름을 지어서 첫 번째 효과로 지정
    .to('.main_title_02', {opacity: 1, y: -50, duration: 4}, 's2')
    .to('.main_title_02', {opacity: 0, y: -100, duration: 4})

    tit.add('s3') // s3 이라는 이름을 지어서 첫 번째 효과로 지정
